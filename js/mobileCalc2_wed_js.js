let tips_group_wed = []; //needs to be completely out of the localstorage loop
let tip_sum_wed;
let tips_group_wed2 = [];
let tip_sum_wed2;
let tips_group_wed3 = [];
...
for(var key in localStorage) {
	...
	if(key.includes('Wed')){
		...
		let atps = document.getElementById('wdd_avr_tps');
		atps.innerHTML 	= averageFinder(json2.tip_amount_num,tips_group_wed,tip_sum_wed);

		//average shift length
		let asl = document.getElementById('wdd_avr_shift');
		asl.innerHTML 	= averageFinder(json2.shift_length,tips_group_wed2,tip_sum_wed2);

		//average tips per hour
		let atph = document.getElementById('wdd_avr_tph');
		atph.innerHTML 	= averageFinder(json2.tips_per_hour,tips_group_wed3,tip_sum_wed3);

		//average tips with hourly
		let ahw = document.getElementById('wdd_avr_hourly');
		ahw.innerHTML 	= averageFinder(json2.tips_hourly_shift,tips_group_wed4,tip_sum_wed4);
		...
		//sum of tips
		let ts 			= document.getElementById('wdd_tip_sum');
		ts.innerHTML 	= sumFinder(json2.tip_amount_num,tips_group_wed5,tip_sum_wed5);

		//get entries box in html
		let entry_main_container 	= document.getElementById('wed_indv_entry_div');
