// localStorage.removeItem('Fri April 27 2018 CustomEntry(id: 25) GMT-0700 (PDT)');

let tips_group_fri = []; //needs to be completely out of the localstorage loop
let tip_sum_fri;
let tips_group_fri2 = [];
let tip_sum_fri2;
let tips_group_fri3 = [];
...
if(key.includes('Fri')){
		...
		let atps = document.getElementById('fr_avr_tps');
		atps.innerHTML 	= averageFinder(json2.tip_amount_num,tips_group_fri,tip_sum_fri);

		//average shift length
		let asl = document.getElementById('fr_avr_shift');
		asl.innerHTML 	= averageFinder(json2.shift_length,tips_group_fri2,tip_sum_fri2);

		//average tips per hour
		let atph = document.getElementById('fr_avr_tph');
		atph.innerHTML 	= averageFinder(json2.tips_per_hour,tips_group_fri3,tip_sum_fri3);

		//average tips with hourly
		let ahw = document.getElementById('fr_avr_hourly');
		ahw.innerHTML 	= averageFinder(json2.tips_hourly_shift,tips_group_fri4,tip_sum_fri4);
		...
		//sum of tips
		let ts 			= document.getElementById('fr_tip_sum');
		ts.innerHTML 	= sumFinder(json2.tip_amount_num,tips_group_fri5,tip_sum_fri5);

		//get entries box in html
		let entry_main_container 	= document.getElementById('fri_indv_entry_div');
		...
		entry_date.style.border 	= '1px solid none';
