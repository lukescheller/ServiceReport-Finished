let tips_group_sun   = []; //needs to be completely out of the localstorage loop
let tip_sum_sun;
let tips_group_sun2 = [];
let tip_sum_sun2;
let tips_group_sun3 = [];
...
for(var key in localStorage) {
	...
	if(key.includes('Sun')){
		...
		let atps = document.getElementById('su_avr_tps');
		atps.innerHTML 	= averageFinder(json2.tip_amount_num,tips_group_sun,tip_sum_sun);

		//average shift length
		let asl = document.getElementById('su_avr_shift');
		asl.innerHTML 	= averageFinder(json2.shift_length,tips_group_sun2,tip_sum_sun2);

		//average tips per hour
		let atph = document.getElementById('su_avr_tph');
		atph.innerHTML 	= averageFinder(json2.tips_per_hour,tips_group_sun3,tip_sum_sun3);

		//average tips with hourly
		let ahw = document.getElementById('su_avr_hourly');
		ahw.innerHTML 	= averageFinder(json2.tips_hourly_shift,tips_group_sun4,tip_sum_sun4);
		...
		//sum of tips
		let ts 			= document.getElementById('su_tip_sum');
		ts.innerHTML 	= sumFinder(json2.tip_amount_num,tips_group_sun5,tip_sum_sun5);

		//get entries box in html
		let entry_main_container 	= document.getElementById('sun_indv_entry_div');
