...
//Create numbers for custom date
let custom_date_select = document.getElementById('custom_date_select');
for(let h=1; h<10; h++){
	let custom_txt = document.createTextNode('0' + h);
	let custom_opt = document.createElement('option');
	custom_opt.appendChild(custom_txt);
	custom_date_select.appendChild(custom_opt);
}

for(let g=10; g<32; g++){
	let custom_txt2 = document.createTextNode(g);
	let custom_opt2 = document.createElement('option');
	custom_opt2.appendChild(custom_txt2);
	custom_date_select.appendChild(custom_opt2);
}

let custom_year_select = document.getElementById('custom_year_select');
for(let l=2018; l<2030; l++){
	let custom_txt3 = document.createTextNode(l);
	let custom_opt3 = document.createElement('option');
	custom_opt3.appendChild(custom_txt3);
	custom_year_select.appendChild(custom_opt3);
}
...
//END TIME CALCULATIONS////////////////////////////////////////////////////////////////
	...
	for(let i=0; i<form[4].value.length; i++){
		for(let t=0; t<specialx.length; t++){
			if(form[4].value[i] == specialx[t]){
	...
	if(form[4].value.startsWith('.')){
		event.preventDefault();
		// alert('works period');
		form_period_start = false; 
	}

	let input_split = form[4].value.split('');
	let input_split_ar = [];
	...
	if(!form[8].checked && !form[9].checked){
		event.preventDefault();
		// alert('works radio')
		form_checked = false;
	}

	for(let i=0; i<form[13].value.length; i++){
		for(let t=0; t<specialx.length; t++){
			if(form[13].value[i] == specialx[t]){
				event.preventDefault();
	...
	if(form[13].value.match(/[a-z]/i)){
		event.preventDefault();
		// alert('works letters 2');
		form_hourly_letters = false;
	}

	let input_splithourly = form[13].value.split('');
	...
	//////custom submit date//////////////////////////////////////////////
	let custom_day_val = document.getElementById('custom_day_select').value;
	if(custom_day_val == 'Monday'){
		custom_day_val = 'Mon';
	}
	if(custom_day_val == 'Tuesday'){
		custom_day_val = 'Tue';
	}
	if(custom_day_val == 'Wednesday'){
		custom_day_val = 'Wed';
	}
	if(custom_day_val == 'Thursday'){
		custom_day_val = 'Thu';
	}
	if(custom_day_val == 'Friday'){
		custom_day_val = 'Fri';
	}
	if(custom_day_val == 'Saturdayday'){
		custom_day_val = 'Sat';
	}
	if(custom_day_val == 'Sunday'){
		custom_day_val = 'Sun';
	}

	let custom_month_val = document.getElementById('custom_month_select').value;
	let custom_date_val = document.getElementById('custom_date_select').value;
	let custom_year_val = document.getElementById('custom_year_select').value;
	let gmt = 'GMT-0700 (PDT)';

	// let custom_calc = 'custom entry' + (localStorage.length + 1) - localStorage.length;
	let custom_subt = Math.random().toFixed(3) * 1000;
	let custom_calc = 'CustomEntry' + '(' + 'id:' + ' ' + custom_subt + ')';

	let date = custom_day_val + ' ' + custom_month_val + ' ' + custom_date_val + ' ' + custom_year_val + ' ' + custom_calc + ' ' + gmt;

	submit_obj = {};

	submit_obj.date 					= date;
