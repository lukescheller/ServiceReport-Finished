let tips_group_thu = []; //needs to be completely out of the localstorage loop
let tip_sum_thu;
let tips_group_thu2 = [];
let tip_sum_thu2;
let tips_group_thu3 = [];
...
for(var key in localStorage) {
	...
	if(key.includes('Thu')){
		...
		let atps = document.getElementById('th_avr_tps');
		atps.innerHTML 	= averageFinder(json2.tip_amount_num,tips_group_thu,tip_sum_thu);

		//average shift length
		let asl = document.getElementById('th_avr_shift');
		asl.innerHTML 	= averageFinder(json2.shift_length,tips_group_thu2,tip_sum_thu2);

		//average tips per hour
		let atph = document.getElementById('th_avr_tph');
		atph.innerHTML 	= averageFinder(json2.tips_per_hour,tips_group_thu3,tip_sum_thu3);

		//average tips with hourly
		let ahw = document.getElementById('th_avr_hourly');
		ahw.innerHTML 	= averageFinder(json2.tips_hourly_shift,tips_group_thu4,tip_sum_thu4);
		...
		//sum of tips
		let ts 			= document.getElementById('th_tip_sum');
		ts.innerHTML 	= sumFinder(json2.tip_amount_num,tips_group_thu5,tip_sum_thu5);

		//get entries box in html
		let entry_main_container 	= document.getElementById('thu_indv_entry_div');
