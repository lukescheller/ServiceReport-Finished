let tips_group_tue = []; //needs to be completely out of the localstorage loop
let tip_sum_tue;
let tips_group_tue2 = [];
let tip_sum_tue2;
let tips_group_tue3 = [];
...
for(var key in localStorage) {
	...
	if(key.includes('Tue')){
		...
		let atps = document.getElementById('td_avr_tps');
		atps.innerHTML 	= averageFinder(json2.tip_amount_num,tips_group_tue,tip_sum_tue);

		//average shift length
		let asl = document.getElementById('td_avr_shift');
		asl.innerHTML 	= averageFinder(json2.shift_length,tips_group_tue2,tip_sum_tue2);

		//average tips per hour
		let atph = document.getElementById('td_avr_tph');
		atph.innerHTML 	= averageFinder(json2.tips_per_hour,tips_group_tue3,tip_sum_tue3);

		//average tips with hourly
		let ahw = document.getElementById('td_avr_hourly');
		ahw.innerHTML 	= averageFinder(json2.tips_hourly_shift,tips_group_tue4,tip_sum_tue4);
		...
		//sum of tips
		let ts 			= document.getElementById('td_tip_sum');
		ts.innerHTML 	= sumFinder(json2.tip_amount_num,tips_group_tue5,tip_sum_tue5);

		//get entries box in html
		let entry_main_container 	= document.getElementById('tue_indv_entry_div');
